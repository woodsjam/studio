...
export function QRCodeLink({ sessionId }: { sessionId: string }) {
  // In a real app, this URL would be a Firebase Dynamic Link
  const linkUrl = `${window.location.origin}/link-faculty?session=${sessionId}`;
  ...
  return (
    <div className="flex flex-col items-center space-y-4 p-4 border rounded-lg bg-muted/50">
      <p className="text-center text-sm text-muted-foreground">
        Scan this QR code with another device to link a faculty or proctor to this session.
      </p>
